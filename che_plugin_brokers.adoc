...
== Plugin brokers
Now, when we have a basic understanding of the `Plugin` and `Plugin Registry` concepts let's slowly move to the `Plugin Broker`.

Plugin Broker - is an application that runs just before the actual workspace start phase and based on the recieved plugin metadata deliveres the plugin binaries to the workspace containers. Taken into account that there are multiple types of plugins, there are also multiple types of brokers:
...
NOTE: Since Che supports plugable editor implementations (Theia, Dirigible, Jupyter Notebook) in future new types of plugins with dedicated brokers could appear e.g. `Dirigible Plugin` / `Dirigible Plugin Broker`  
