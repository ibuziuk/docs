...
This metadata is consumed by Che sever and exposed via the User Dashboard for plugin picking during the workspace creation:

image::images/che-plugin-brokers/plugins.png[width="1640", height="560",aption="Plugin selection on User Dashboard"]

NOTE: During workspace creation user can select only one `Che Editor Plugin` and multiple `Che`, `VS Code`, `Theia` plugins.
...
NOTE: `Theia IDE` support both `VSCode extension` and `Theia` plugins
...
NOTE: Since Che supports plugable editor implementations (Theia, Dirigible, Jupyter Notebook) in future it is planned to have an ability to add broker of plugins suitable for a new editor e.g. `Dirigible Plugin Broker`  
